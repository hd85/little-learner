...
function PageShell({ onBack, title, emoji, subtitle, children, mascotMood, totalStars, companionContext }) {
...
function CountingGame({ onBack, onComplete, totalStars }) {
  const [round, setRound] = useState(0); const [target, setTarget] = useState(0); const [items, setItems] = useState([]); const [options, setOptions] = useState([]); const [selected, setSelected] = useState(null); const [correct, setCorrect] = useState(null); const [celebrating, setCelebrating] = useState(false); const [score, setScore] = useState(0); const [mascotMood, setMascotMood] = useState("idle"); const [correctStreak, setCorrectStreak] = useState(0); const [wrongStreak, setWrongStreak] = useState(0); const [showHint, setShowHint] = useState(false); const T = 6;
  const newRound = useCallback(() => { const c = randInt(1, 9); const es = [ANIMALS_E, FRUITS, NATURE, VEHICLES_E, SPORTS_E][randInt(0, 4)]; const em = es[randInt(0, es.length - 1)]; setTarget(c); setItems(Array.from({ length: c }, (_, i) => ({ emoji: em, id: i, x: 15 + Math.random() * 70, y: 10 + Math.random() * 60, delay: i * 0.1, rotation: -15 + Math.random() * 30 }))); setSelected(null); setCorrect(null); setMascotMood("idle"); setShowHint(false); const w = new Set(); while (w.size < 2) { const x = randInt(1, 9); if (x !== c) w.add(x); } setOptions(shuffle([c, ...w])); }, []);
  useEffect(() => { newRound(); }, []);
  const pick = (n) => { if (selected !== null) return; setSelected(n); playTap(); if (n === target) { const ns = score + 1; setScore(ns); setCorrect(true); setCelebrating(true); setMascotMood("happy"); setCorrectStreak(s => s + 1); setWrongStreak(0); setShowHint(false); playCorrect(); setTimeout(() => { setCelebrating(false); if (round < T - 1) { setRound(r => r + 1); newRound(); } else { onComplete?.(ns, T); } }, 1800); } else { const nws = wrongStreak + 1; setCorrect(false); setMascotMood("encourage"); setWrongStreak(nws); setCorrectStreak(0); if (nws >= 2) setShowHint(true); playWrong(); setTimeout(() => { setSelected(null); setCorrect(null); setMascotMood("idle"); }, 1200); } };
  return <PageShell onBack={onBack} title="Count with Me" emoji="🌿" subtitle="How many do you see?" mascotMood={mascotMood} totalStars={totalStars} companionContext={{ correctStreak, wrongStreak }}><ProgressDots total={T} current={round} /><div style={{ background: theme.card, borderRadius: 28, padding: 24, boxShadow: theme.shadow, position: "relative", height: 260, marginTop: 16, overflow: "hidden" }}>{items.map(i => <span key={i.id} style={{ position: "absolute", left: `${i.x}%`, top: `${i.y}%`, fontSize: 42, transform: `rotate(${i.rotation}deg)`, animation: `popIn 0.5s ease backwards${showHint ? ", pulse 1s ease-in-out infinite" : ""}`, animationDelay: `${i.delay}s`, userSelect: "none" }}>{i.emoji}</span>)}</div><div style={{ display: "flex", gap: 16, justifyContent: "center", marginTop: 28 }}>{options.map(n => { const s = selected === n, r = correct === true && s, w = correct === false && s; return <button key={n} onClick={() => pick(n)} style={{ width: 80, height: 80, borderRadius: 24, background: r ? "#D4EDDA" : w ? "#F8D7DA" : theme.card, border: r ? `3px solid ${theme.accent2}` : w ? `3px solid ${theme.accent3}` : "3px solid #E8DDD0", fontFamily: "'Baloo 2', cursive", fontSize: 32, fontWeight: 700, color: theme.text, cursor: selected !== null ? "default" : "pointer", transform: s ? "scale(1.1)" : "scale(1)", transition: "all 0.25s ease", boxShadow: theme.shadow, animation: w ? "wiggle 0.3s ease" : "none" }}>{n}</button>; })}</div>{round >= T - 1 && correct === true && <div style={{ textAlign: "center", marginTop: 12 }}><StarDisplay score={score} total={T} /><ValueMoment activityId="counting" /><p style={{ fontFamily: "'Quicksand', sans-serif", fontSize: 14, color: theme.textLight, margin: "8px 0" }}>{ENCOURAGEMENT.effort[randInt(0, ENCOURAGEMENT.effort.length - 1)]}</p><GentleButton onClick={() => { setRound(0); setScore(0); setCorrectStreak(0); setWrongStreak(0); newRound(); }}>Play Again 🌻</GentleButton></div>}<Celebration show={celebrating} /></PageShell>;
}
...
function LetterMatch({ onBack, onComplete, totalStars }) {
  const [round, setRound] = useState(0); const [puzzle, setPuzzle] = useState(null); const [selected, setSelected] = useState(null); const [correct, setCorrect] = useState(null); const [celebrating, setCelebrating] = useState(false); const [score, setScore] = useState(0); const [mascotMood, setMascotMood] = useState("idle"); const [correctStreak, setCorrectStreak] = useState(0); const [wrongStreak, setWrongStreak] = useState(0); const [showHint, setShowHint] = useState(false); const T = 6;
  const newRound = useCallback(() => { const i = randInt(0, 25); const t = PHONICS_DATA[i]; const w = new Set(); while (w.size < 2) { const x = randInt(0, 25); if (x !== i) w.add(x); } setPuzzle({ target: t, options: shuffle([t, ...Array.from(w).map(x => PHONICS_DATA[x])]) }); setSelected(null); setCorrect(null); setMascotMood("idle"); setShowHint(false); }, []);
...
function AnimalHabitats({ onBack, onComplete, totalStars }) {
  const [round, setRound] = useState(0); const [puzzle, setPuzzle] = useState(null); const [selected, setSelected] = useState(null); const [correct, setCorrect] = useState(null); const [celebrating, setCelebrating] = useState(false); const [score, setScore] = useState(0); const [mascotMood, setMascotMood] = useState("idle"); const [correctStreak, setCorrectStreak] = useState(0); const [wrongStreak, setWrongStreak] = useState(0); const T = 8;
  const newRound = useCallback(() => { const hi = randInt(0, HABITATS.length - 1); const h = HABITATS[hi]; const a = h.animals[randInt(0, h.animals.length - 1)]; const opts = shuffle([h, ...shuffle(HABITATS.filter((_, i) => i !== hi)).slice(0, 2)]); setPuzzle({ animal: a, correctHabitat: h, options: opts }); setSelected(null); setCorrect(null); setMascotMood("idle"); }, []);
  useEffect(() => { newRound(); }, []); if (!puzzle) return null;
  const pick = (n) => { if (selected !== null) return; setSelected(n); playTap(); if (n === puzzle.correctHabitat.name) { const ns = score + 1; setScore(ns); setCorrect(true); setCelebrating(true); setMascotMood("happy"); setCorrectStreak(s => s + 1); setWrongStreak(0); playCorrect(); setTimeout(() => { setCelebrating(false); if (round < T - 1) { setRound(r => r + 1); newRound(); } else { onComplete?.(ns, T); } }, 2000); } else { setCorrect(false); setMascotMood("encourage"); setWrongStreak(s => s + 1); setCorrectStreak(0); playWrong(); setTimeout(() => { setSelected(null); setCorrect(null); setMascotMood("idle"); }, 1200); } };
  return <PageShell onBack={onBack} title="Animal Homes" emoji="🌍" subtitle="Where does this animal live?" mascotMood={mascotMood} totalStars={totalStars} companionContext={{ correctStreak, wrongStreak }}><ProgressDots total={T} current={round} /><div style={{ background: theme.card, borderRadius: 28, padding: "32px 24px", boxShadow: theme.shadow, textAlign: "center", marginTop: 16 }}><div style={{ fontSize: 90, lineHeight: 1, animation: "popIn 0.5s ease" }}>{puzzle.animal.emoji}</div><p style={{ fontFamily: "'Baloo 2', cursive", fontSize: 28, color: theme.text, marginTop: 12 }}>{puzzle.animal.name}</p></div><div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 24 }}>{puzzle.options.map(h => { const s = selected === h.name, r = correct === true && s, w = correct === false && s; return <button key={h.name} onClick={() => pick(h.name)} style={{ background: r ? "#D4EDDA" : w ? "#F8D7DA" : h.bg, border: r ? `3px solid ${theme.accent2}` : w ? `3px solid ${theme.accent3}` : "3px solid transparent", borderRadius: 20, padding: "16px 24px", display: "flex", alignItems: "center", gap: 16, cursor: selected ? "default" : "pointer", transition: "all 0.25s ease", transform: s ? "scale(1.03)" : "scale(1)", boxShadow: theme.shadow, animation: w ? "wiggle 0.3s ease" : "none" }}><span style={{ fontSize: 36 }}>{h.emoji}</span><span style={{ fontFamily: "'Baloo 2', cursive", fontSize: 22, fontWeight: 700, color: theme.text }}>{h.name}</span><div style={{ marginLeft: "auto", display: "flex", gap: 4 }}>{h.animals.slice(0, 3).map((a, i) => <span key={i} style={{ fontSize: 20 }}>{a.emoji}</span>)}</div></button>; })}</div>{round >= T - 1 && correct === true && <div style={{ textAlign: "center", marginTop: 12 }}><StarDisplay score={score} total={T} /><ValueMoment activityId="habitats" /><p style={{ fontFamily: "'Quicksand', sans-serif", fontSize: 14, color: theme.textLight, margin: "8px 0" }}>{ENCOURAGEMENT.effort[randInt(0, ENCOURAGEMENT.effort.length - 1)]}</p><GentleButton onClick={() => { setRound(0); setScore(0); setCorrectStreak(0); setWrongStreak(0); newRound(); }}>Play Again 🌍</GentleButton></div>}<Celebration show={celebrating} /></PageShell>;
}
...
function ColorMixer({ onBack, onComplete, totalStars }) {
  ...
  return <PageShell onBack={onBack} title="Color Mixer" emoji="🎨" subtitle="Mix colors to make new ones!" mascotMood={mascotMood} totalStars={totalStars} companionContext={{ correctStreak, wrongStreak }}>
    <ProgressDots total={T} current={round} />
...
function PatternGame({ onBack, onComplete, totalStars }) {
  const [round, setRound] = useState(0); const [puzzle, setPuzzle] = useState(null); const [selected, setSelected] = useState(null); const [correct, setCorrect] = useState(null); const [celebrating, setCelebrating] = useState(false); const [score, setScore] = useState(0); const [mascotMood, setMascotMood] = useState("idle"); const [correctStreak, setCorrectStreak] = useState(0); const [wrongStreak, setWrongStreak] = useState(0); const [showHint, setShowHint] = useState(false); const T = 8;
  const usedRef = useRef(new Set());
  const newRound = useCallback(() => { let idx; do { idx = randInt(0, PATTERNS.length - 1); } while (usedRef.current.has(idx) && usedRef.current.size < PATTERNS.length); usedRef.current.add(idx); const p = PATTERNS[idx]; setPuzzle({ ...p, options: shuffle(p.options) }); setSelected(null); setCorrect(null); setMascotMood("idle"); }, []);
  useEffect(() => { newRound(); }, []);
  if (!puzzle) return null;
  ...
  return <PageShell onBack={onBack} title="What Comes Next?" emoji="🧩" subtitle="Find the pattern!" mascotMood={mascotMood} totalStars={totalStars} companionContext={{ correctStreak, wrongStreak }}>
    ...
    <div style={{ display: "flex", gap: 16, justifyContent: "center", marginTop: 28 }}>
      {puzzle.options.map((opt, i) => {
        const isSel = selected === opt; const isRight = correct === true && isSel; const isWrong = correct === false && isSel;
        return <button key={i} onClick={() => pick(opt)} style={{ width: 80, height: 80, borderRadius: 24, background: isRight ? "#D4EDDA" : isWrong ? "#F8D7DA" : theme.card, border: isRight ? `3px solid ${theme.accent2}` : isWrong ? `3px solid ${theme.accent3}` : "3px solid #E8DDD0", fontSize: 36, cursor: selected ? "default" : "pointer", transform: isSel ? "scale(1.1)" : "scale(1)", transition: "all 0.25s ease", boxShadow: theme.shadow, animation: isWrong ? "wiggle 0.3s ease" : "none" }}>{opt}</button>;
      })}
    </div>
  ...
  return <PageShell onBack={onBack} title="Odd One Out" emoji="🔍" subtitle="Which one is different?" mascotMood={mascotMood} totalStars={totalStars} companionContext={{ correctStreak, wrongStreak }}>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 16, marginTop: 24, maxWidth: 320, marginLeft: "auto", marginRight: "auto" }}>
      {puzzle.items.map((item, i) => {
        ...
        return <button key={i} onClick={() => tap(item, i)} style={{ aspectRatio: "1", borderRadius: 28, background: isRight ? "#D4EDDA" : isWrong ? "#F8D7DA" : isGroupHighlight ? theme.accent2 + "22" : theme.card, border: isRight ? `3px solid ${theme.accent2}` : isWrong ? `3px solid ${theme.accent3}` : isGroupHighlight ? `3px solid ${theme.accent2}44` : "3px solid #EDE6DC", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 56, cursor: selected !== null ? "default" : "pointer", transition: "all 0.3s ease", animation: `popIn 0.4s ease backwards ${isWrong ? ", wiggle 0.3s ease" : ""}`, animationDelay: `${i * 0.08}s`, transform: isSel ? "scale(1.05)" : "scale(1)", boxShadow: theme.shadow, opacity: isHintDim ? 0.5 : 1 }}>{item.emoji}</button>;
      })}
    </div>
